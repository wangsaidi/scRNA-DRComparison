#!/usr/bin/env python3
"""Run scGBM dimension-sensitivity controls on local synthetic datasets."""
import argparse
import json
import resource
import subprocess
...
R_HELPER = Path("revision_benchmark/scripts/run_scgbm_embedding.R")
...
def run_one(dataset_id: str, dimension: int, seed: int, output_root: Path, metric_sample_size: int, max_iter: int, min_iter: int) -> dict:
    ...
    emb_dir = output_root / "embeddings" / "WP2_dimension_sensitivity" / dataset_id / "scGBM" / f"dim_{dimension}"
    ...
    if emb.isna().any().any():
        raise ValueError(f"scGBM embedding {emb_path} does not align to matrix cells")
    ...
    metric_path = metric_dir / "WP2_dimension_sensitivity_scGBM_metrics.csv"
    ...
    log_path = log_dir / f"{dataset_id}_scGBM_dim{dimension}_seed{seed}.json"
...
def main() -> int:
    ...
    failed_path = Path(args.output_root) / "logs" / "WP2_dimension_sensitivity_scGBM_failed_runs.jsonl"
    for dataset_id in datasets:
        for dim in dims:
            for seed in seeds:
                print(f"RUN scGBM dataset={dataset_id} dimension={dim} seed={seed}", flush=True)
                try:
