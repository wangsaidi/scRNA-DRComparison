#!/usr/bin/env python3
"""Run SAUCIE dimension-sensitivity controls on local synthetic datasets."""
import argparse
import json
import resource
import sys
...
sys.path.insert(0, str(Path("revision_benchmark/sources").resolve()))
...
def run_one(dataset_id: str, dimension: int, seed: int, output_root: Path, metric_sample_size: int, steps: int) -> dict:
    ...
    hidden1 = min(512, max(64, values.shape[1] // 4))
    hidden2 = min(256, max(32, hidden1 // 2))
    hidden3 = min(128, max(16, hidden2 // 2))
    layers = [hidden1, hidden2, hidden3, dimension]

    model = SAUCIE.SAUCIE(values.shape[1], layers=layers, no_gpu=True)
    train_loader = SAUCIE.Loader(values, shuffle=True)
    model.train(train_loader, steps=steps)
    z = np.asarray(model.get_embedding(SAUCIE.Loader(values, shuffle=False)), dtype=np.float64)

    emb_dir = output_root / "embeddings" / "WP2_dimension_sensitivity" / dataset_id / "SAUCIE" / f"dim_{dimension}"
    ...
    pd.DataFrame(z, index=x.index, columns=[f"SAUCIE_{i + 1}" for i in range(z.shape[1])]).to_csv(emb_path)

    metrics = compute_metrics(values, z, labels, seed, metric_sample_size)
    metrics.update({
    ...
    metric_path = metric_dir / "WP2_dimension_sensitivity_SAUCIE_metrics.csv"
    row = pd.DataFrame([metrics])
    if metric_path.exists():
        old = pd.read_csv(metric_path)
        row = pd.concat([old, row], ignore_index=True)
        row = row.drop_duplicates(subset=["dataset_id", "method", "dimension", "seed", "training_steps"], keep="last")
    row.to_csv(metric_path, index=False)

    log_path = log_dir / f"{dataset_id}_SAUCIE_dim{dimension}_seed{seed}.json"
...
def main() -> int:
    ...
    failed_path = Path(args.output_root) / "logs" / "WP2_dimension_sensitivity_SAUCIE_failed_runs.jsonl"
    for dataset_id in datasets:
        for dim in dims:
            for seed in seeds:
                print(f"RUN SAUCIE dataset={dataset_id} dimension={dim} seed={seed}", flush=True)
                try:
                    metrics = run_one(dataset_id, dim, seed, Path(args.output_root), args.metric_sample_size, args.steps)
