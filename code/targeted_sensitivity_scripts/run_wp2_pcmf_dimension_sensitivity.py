#!/usr/bin/env python3
"""Run pCMF dimension-sensitivity controls on local synthetic datasets."""
import argparse
import json
import resource
import subprocess
...
R_HELPER = Path("revision_benchmark/scripts/run_pcmf_embedding.R")
...
def run_one(
    ...
    emb_dir = output_root / "embeddings" / "WP2_dimension_sensitivity" / dataset_id / "pCMF" / f"dim_{dimension}"
    ...
    if emb.isna().any().any():
        raise ValueError(f"pCMF embedding {emb_path} does not align to matrix cells")
    ...
    metric_path = metric_dir / "WP2_dimension_sensitivity_pCMF_metrics.csv"
    row = pd.DataFrame([metrics])
    if metric_path.exists():
        old = pd.read_csv(metric_path)
        row = pd.concat([old, row], ignore_index=True)
        row = row.drop_duplicates(subset=["dataset_id", "method", "dimension", "seed", "iter_max", "pseudocount"], keep="last")
    row.to_csv(metric_path, index=False)

    log_path = log_dir / f"{dataset_id}_pCMF_dim{dimension}_seed{seed}.json"
...
def main() -> int:
    ...
    parser.add_argument("--iter-min", type=int, default=20)
    ...
    failed_path = Path(args.output_root) / "logs" / "WP2_dimension_sensitivity_pCMF_failed_runs.jsonl"
    for dataset_id in datasets:
        for dim in dims:
            for seed in seeds:
                print(f"RUN pCMF dataset={dataset_id} dimension={dim} seed={seed}", flush=True)
                try:
                    metrics = run_one(
                        dataset_id,
                        dim,
