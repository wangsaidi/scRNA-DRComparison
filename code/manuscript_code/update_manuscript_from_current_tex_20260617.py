...
def clean_tex_source() -> str:
    ...
    tex = tex.replace(
        ...
        "%% Manuscript generated from the project-local latest clean LaTeX and synchronized with final figures/tables on 2026-06-17.",
    )
    tex = normalize_figure_reference_style(tex)
    tex = keep_main_figures_with_legends(tex)
    return tex


def keep_main_figures_with_legends(text: str) -> str:
    """Place each main figure large on the page and allow long legends to paginate."""
    for fig_no, filename in FIGURES.items():
        pattern = re.compile(
            r"\\clearpage\s*"
            r"\\begin\{center\}\s*"
            ...
            r"(?P<caption>\\captionof\{figure\}\{.*?\})\s*"
            r"(?P<label>\\label\{fig:fig"
            + str(fig_no)
            + r"\})\s*"
            r"\\end\{minipage\}\s*"
            r"\\clearpage",
        ...
        height = FIGURE_DISPLAY_HEIGHTS[fig_no]

        def repl(match: re.Match[str]) -> str:
            label = match.group("label") or f"\\label{{fig:fig{fig_no}}}"
            caption_text = match.group("caption")
            caption_text = caption_text.strip()
            if caption_text.startswith(r"\captionof{figure}{"):
                caption_text = re.sub(r"^\\captionof\{figure\}\{", "", caption_text, count=1, flags=re.S)
                if caption_text.endswith("}"):
                    caption_text = caption_text[:-1]
            elif caption_text.startswith(r"\noindent{\scriptsize\textbf{Figure~\thefigure.}"):
                caption_text = re.sub(
                    r"^\\noindent\{\\scriptsize\\textbf\{Figure~\\thefigure\.\}\s*",
                    "",
                    caption_text,
                    count=1,
                    flags=re.S,
                )
                caption_text = re.sub(r"\}\\par$", "", caption_text, count=1, flags=re.S)
            return (
                "\\clearpage\n"
                "\\begingroup\n"
                ...
                f"{label}\n"
                "\\begin{center}\n"
                f"\\makebox[\\linewidth][c]{{\\includegraphics[width={FIGURE_DISPLAY_WIDTH}\\paperwidth,height={height}\\textheight,keepaspectratio]{{figures/{filename}}}}}\n"
                "\\end{center}\n"
                "\\vspace{-0.25em}\n"
                f"\\noindent{{\\scriptsize\\textbf{{Figure~\\thefigure.}} {caption_text}}}\\par\n"
                "\\par\\endgroup\n"
                "\\clearpage"
            )
        ...
        if count != 1:
            rewritten_pattern = re.compile(
                r"\\clearpage\s*"
                r"\\begingroup\s*"
                r"\\(?:captionsetup\{type=figure,font=scriptsize,labelfont=bf,skip=4pt,hypcap=false\}\s*)?"
                ...
                r"(?:\\end\{center\}\s*)?"
                r"(?:\\vspace\{[-0-9.]+em\}\s*)?"
                r"(?P<caption>(?:\\captionof\{figure\}\{.*?\}|\\noindent\{\\scriptsize\\textbf\{Figure~\\thefigure\.\}.*?\}\\par))\s*"
                r"(?:\\label\{fig:fig"
                + str(fig_no)
                + r"\})?\s*"
                r"(?:\\end\{minipage\}\s*)?"
            ...
            text, count = rewritten_pattern.subn(repl, text, count=1)
        if count != 1:
            already_rewritten = (
                f"height={height}\\textheight,keepaspectratio]{{figures/{filename}}}" in text
                and f"\\label{{fig:fig{fig_no}}}" in text
                and "\\refstepcounter{figure}" in text
            )
            if not already_rewritten:
                raise RuntimeError(f"Could not rewrite Figure {fig_no} image-caption block.")
    return text
...
def make_word_friendly_tex(tex: str) -> str:
    """Convert PDF-oriented large-figure blocks to standard figure floats for Pandoc."""
    for fig_no, filename in FIGURES.items():
        pattern = re.compile(
            r"\\clearpage\s*"
            r"\\begingroup\s*"
        ...
        if count != 1:
            raise RuntimeError(f"Could not make Figure {fig_no} Word-friendly.")
    return tex
...
def main() -> None:
    ...
    word_tex = make_word_friendly_tex(clean)
    word_tex_path = LATEX_CLEAN / "manuscript_final_clean_for_word.tex"
    ...
    convert_word(LATEX_CLEAN, word_tex_path.name, clean_docx, "clean_update_20260616")
    prefix_word_captions(clean_docx)
    shutil.copy2(clean_docx, red_docx)
