...
'''
指标计算（局部监督评估：KNN分类准确率）
1.使用k-最近邻（k-nearest neighbors）分类器评估降维嵌入的质量。
2.通过交叉验证（cross-validation）计算平均准确率。
'''
...
'''
1.类似于knnn_eval，但使用数据子集进行评估，适用于大数据集
2.为了提高计算效率，它在每个测试集中只使用一部分样本进行评估（采样，一部分数据）
'''
...
'''
1.对一系列的n_neighbors值运行knn_eval函数，生成准确率系列
2.计算多个不同 n_neighbors 值下的 kNN 分类准确率，用于评估低维嵌入数据在不同参数设置下的分类性能
还是一种降维结果，只不过是多种n_neighbors
'''
...
'''
指标计算（局部监督评估：SVM分类准确率）
1.使用支持向量机（SVM）评估降维嵌入的质量
2.通过交叉验证计算的平均分类准确率
'''
...
'''
1.类似于svm_eval，但使用数据子集进行评估，适用于大数据集
2.为了提高计算效率，它在每个测试集中只使用一部分样本进行评估（采样，一部分数据）
'''
...
'''
1.类似于svm_eval，但使用加速的SVM评估
2.这个函数也是使用 SVM 来评估低维嵌入数据的分类性能，但使用了 Nystroem 方法来近似 RBF 核，从而提高计算效率
'''
...
'''
指标计算（Random Triplet Accuracy）
1.评估降维嵌入的全局结构，通过计算随机三元组（triplets）的满意度
'''
...
'''
指标计算（局部无监督、最近邻保留率）
1.评估局部结构保留，通过比较高维空间和低维空间中的最近邻域
2.这个函数计算的是 全局最近邻保留率，适用于较小的数据集
计算单个低维嵌入的最近临保留率
'''
...
'''
指标计算（局部无监督、最近邻保留率）
1.类似于neighbor_kept_ratio_eval，但使用数据子集进行评估，适用于大数据集
2.这个函数计算的是 采样最近邻保留率，适用于较大的数据集
'''
...
'''
1.对一系列的低维嵌入运行neighbor_kept_ratio_eval函数
2.计算多个低维嵌入数据的最近邻保留率
与neighbor_kept_ratio_eval一样，计算的都是最近临保留率，只不过，neighbor_kept_ratio_eval是计算单个低维嵌入，这两个方法是计算多个低维嵌入的最近临保留率
'''
...
'''
1.类似于neighbor_kept_ratio_series_eval，但使用更快的方法
2.计算多个低维嵌入数据的最近邻保留率，但使用了更高效的实现方式
与neighbor_kept_ratio_eval一样，计算的都是最近临保留率，只不过，neighbor_kept_ratio_eval是计算单个低维嵌入，这两个方法是计算多个低维嵌入的最近临保留率
'''
...
'''
调用
1.对指定数据集和方法列表运行一系列评估函数
调用了neighbor_kept_ratio_series_eval_fast（_nkratios）、random_triplet_eval（_rteratios）、knn_eval（_knnaccs）、svm_eval（_svmaccs）
'''
...
调用
1.针对非监督评估，运行一系列评估函数
这里头运行了neighbor_kept_ratio_series_eval_fast（_nkratios）、random_triplet_eval（_rteratios）
'''
...
指标计算（1%最近邻保留率）
1.仅使用数据集的1%运行评估函数
2.用于评估降维方法在保留数据局部结构方面的能力。具体来说，它衡量的是在降维后的低维空间中，原始高维空间中的1%最近邻是否被保留。
'''
...
'''
指标计算（Distance Spearman Correlation）
1.评估降维嵌入的全局结构，通过计算Spearman相关系数
2.它衡量的是高维空间和低维嵌入空间中样本点之间的距离关系是否被保留
'''
...
'''
调用
1.对一系列的低维嵌入运行spearman_correlation_eval函数
'''
...
'''
指标计算（k-Nearest Classes Preservation）
1.评估降维嵌入的全局结构，通过比较聚类中心的最近邻域
2.KNC指标衡量的是在原始高维空间和低维嵌入空间中，每个类别（cluster）的中心点的最近邻类别是否被保留。它特别关注类别之间的相对位置关系，而不是单个数据点的局部结构
'''
def centroid_knn_eval(X, X_new, y, k):
    ...
    nbrs = NearestNeighbors(n_neighbors=k+1).fit(cluster_mean_ori)  # 原
    # nbrs = NearestNeighbors(n_neighbors=min(k+1,num_cat)).fit(cluster_mean_ori)  # 改
    _, indices = nbrs.kneighbors(cluster_mean_ori)
    indices = indices[:,1:] # Remove the center itself

    # Now for the low dimension
    nbr_low = NearestNeighbors(n_neighbors=k+1).fit(cluster_mean_new)  # 原
    # nbr_low = NearestNeighbors(n_neighbors=min(k+1,num_cat)).fit(cluster_mean_new)  # 改
...
'''
调用
1.对一系列的低维嵌入运行centroid_knn_eval函数
'''
...
'''
指标计算（Centroid Distance Correlation）
1.类似于centroid_knn_eval，但计算Spearman相关系数
2.这个函数用于评估降维嵌入的全局结构，特别是聚类中心的邻域保留情况。它通过计算高维空间和低维嵌入空间中聚类中心之间的距离矩阵，并使用Spearman相关系数来衡量这两个距离矩阵之间的相关性
3.计算类别中心点距离矩阵的Spearman相关系数，进一步评估全局结构（Centroid Distance Correlation）
'''
...
'''
调用
1.运行额外的评估函数，包括Spearman相关和聚类中心的最近邻域
2.它用于运行一系列评估方法来评估降维嵌入的质量。它不仅包括了centroid_corr_eval函数，还可能包括其他评估方法
3.目前并不包括，可以扩展，加入其他指标的计算；
这个里头运行了centroid_corr_eval（_centroidcorr）
'''
...
'''
调用
1.类似于eval_reduction_additional_ii，但运行额外的评估函数
这个里头运行了spearman_correlation_series_eval（_spearmancorr）、centroid_knn_series_eval（_centroidknn）
'''
...
调用
1.针对大数据集运行一系列评估函数
2.Unsupervised eval：NK Ratio、RT Ratio；Supervised eval：KNN Acc、SVM Acc
这个里头运行了neighbor_kept_ratio_eval_large（_nkratios）、random_triplet_eval（_rteratios）、knn_eval_large（_knnaccs）、svm_eval_large（_svmaccs）
'''
...
'''
输入数据都是 细胞 x 基因
AJI指标计算，可以修改k值为10、20、30分别计算
局部无监督指标
'''
def average_jaccard_index(X_high, X_low, k=10, metric='euclidean'):
    """
    计算降维前后邻居集合的 Average Jaccard Index（AJI）

    参数：
    ----------
    X_high : ndarray of shape (n_samples, n_features_high)
        高维空间中的原始数据（如基因表达矩阵，降维前）

    X_low : ndarray of shape (n_samples, n_features_low)
        低维空间中的数据（降维结果，例如 t-SNE / UMAP / PCA 输出）

    k : int, default=10
        用于构造邻居集合的邻居数，即每个点在高低维空间中分别取前 k 个邻居

    metric : str, default='euclidean'
        邻居搜索时使用的距离度量方式（传给 NearestNeighbors，如 'euclidean', 'cosine' 等）

    返回：
    ----------
    avg_ji : float
        所有样本点的 Jaccard Index 的平均值（Average Jaccard Index）
    ...
    # 获取低维空间的前 k 邻居（不包括自身）
    nn_low = NearestNeighbors(n_neighbors=k+1, metric=metric).fit(X_low)
    neighbors_low = nn_low.kneighbors(return_distance=False)[:, 1:]

    # 计算每个样本的 Jaccard Index
    jaccard_scores = []
    for i in range(n_samples):
        set_high = set(neighbors_high[i])
        set_low = set(neighbors_low[i])
        intersection = set_high & set_low
        union = set_high | set_low
        ji = len(intersection) / len(union) if union else 1.0  # 避免除0
        jaccard_scores.append(ji)

    # 返回平均值
    avg_ji = np.mean(jaccard_scores)
    return avg_ji
# ================================================================================================== 8
